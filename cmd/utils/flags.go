...
var (
	...
	EspressoFlag = cli.BoolFlag{
		Name:  "espresso",
		Usage: "Espresso network: pre-configured proof-of-authority shortlived test network.",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(EspressoFlag.Name) {
			return filepath.Join(path, "espresso")
		}
		return path
	}
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(EspressoFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "espresso")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, LegacyTestnetFlag, RopstenFlag, RinkebyFlag, GoerliFlag, EspressoFlag)
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(EspressoFlag.Name):
		genesis = core.DefaultEspressoGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
